...
def plot_results(data, save_path='results'):
    ...
    fig.suptitle('Fully Connected Layer with Current Mirror', fontsize=14, fontweight='bold')

    # Panel 1: V(in) - Input from 3 neurons
    axes[0].plot(t_ms, data['V_neuron_out'][0]*1e3, '#E91E63', lw=1)
    axes[0].plot(t_ms, data['V_neuron_out'][1]*1e3, '#9C27B0', lw=1)
    axes[0].plot(t_ms, data['V_neuron_out'][2]*1e3, '#2196F3', lw=1)
    axes[0].set_ylabel('V(in) [mV]')
    axes[0].set_ylim(0, max(100, np.max(data['V_neuron_out'][0])*1e3*1.1))
    axes[0].grid(True, alpha=0.3)

    # Panel 2: V(gate)
    axes[1].plot(t_ms, data['V_gate_out'][0]*1e3, '#F44336', lw=1)
    axes[1].plot(t_ms, data['V_gate_out'][1]*1e3, '#4CAF50', lw=1)
    axes[1].plot(t_ms, data['V_gate_out'][2]*1e3, '#2196F3', lw=1)
    ...
    axes[2].plot(t_ms, V_cm, '#9C27B0', lw=1)
    ...
    axes[3].plot(t_ms, V_out, '#2196F3', lw=1)
    ...
    axes[4].plot(t_ms, I_rout2_uA, '#4CAF50', lw=1)
    axes[4].set_ylabel('I(Rout2) [µA]')
    axes[4].set_ylim(0, max(1, np.max(I_rout2_uA) * 1.2))
    axes[4].grid(True, alpha=0.3)

    # Panel 6: I(Rout3)
    I_rout3_nA = np.abs(data['I_Rout3']) * 1e9
    axes[5].plot(t_ms, I_rout3_nA, '#E91E63', lw=1)
    axes[5].set_ylabel('I(Rout3) [nA]')
    axes[5].set_xlabel('Time [ms]')
    axes[5].set_ylim(0, max(100, np.max(I_rout3_nA) * 1.1))
