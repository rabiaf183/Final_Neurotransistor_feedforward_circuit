...
def plot_results(data, save_path='results'):
    ...
    fig.suptitle('Neurotransistor Behaviour with Current Mirror', fontsize=16, fontweight='bold', y=0.995)

    # Panel 1: Input Pulse
    axes[0].plot(t_ms, data['V_input'][0], 'r', lw=1, label='V(in0)')
    axes[0].plot(t_ms, data['V_input'][1], 'g', lw=1, label='V(in1)')
    axes[0].plot(t_ms, data['V_input'][2], 'b', lw=1, label='V(in2)')
    ...
    axes[1].plot(t_ms, data['V_gate'][0]*1e3, 'r', lw=1, label='V(gate0)')
    axes[1].plot(t_ms, data['V_gate'][1]*1e3, 'g', lw=1, label='V(gate1)')
    axes[1].plot(t_ms, data['V_gate'][2]*1e3, 'b', lw=1, label='V(gate2)')
    ...
    axes[2].plot(t_ms, np.abs(data['I_Rout'])*1e6, 'g', lw=1)
    ...
    # Panel 4: V(Rout)
    axes[3].plot(t_ms, data['V_cm_in']*1e3, 'r', lw=1)
