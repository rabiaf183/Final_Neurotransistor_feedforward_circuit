...
def plot_results(data, save_path='results'):
    ...
    t_us = data['t'] * 1e6  # Time in µs

    fig, axes = plt.subplots(6, 1, figsize=(14, 16), sharex=True)
    fig.suptitle(f'Neurotransistor Behaviour with Current Mirror - MNIST Digit {digit}', fontsize=16, fontweight='bold', y=0.995)

    colors_input = ['#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00', '#a65628']

    # Panel 1: Input Pulse
    for i in range(N):
        axes[0].plot(t_us, data['V_input'][i], color=colors_input[i % len(colors_input)], lw=0.8, alpha=0.8)
    ...
    axes[1].plot(t_us, data['V_gate'][0]*1e3, 'r', lw=1, label='V(gate0)')
    axes[1].plot(t_us, data['V_gate'][1]*1e3, 'g', lw=1, label='V(gate1)')
    axes[1].plot(t_us, data['V_gate'][2]*1e3, 'b', lw=1, label='V(gate2)')
    axes[1].axhline(NMOS_Vto*1e3, color='k', ls='--', lw=2, label='Vth')
    axes[1].set_ylabel('Voltage (mV)')
    axes[1].set_ylim(0, 400)
    axes[1].legend(loc='upper left')
    axes[1].set_title('Gate Voltages', fontsize=11)
    axes[1].grid(True, alpha=0.3)

    # Panel 3: Spike Current I(Rout)
    axes[2].plot(t_us, np.abs(data['I_Rout'])*1e6, 'g', lw=1)
    axes[2].set_ylabel('Current (µA)')
    axes[2].set_ylim(0, max(1, np.max(np.abs(data['I_Rout']))*1e6*1.2))
    axes[2].set_title('Spike Current: I(Rout)', fontsize=11)
    axes[2].grid(True, alpha=0.3)

    # Panel 4: V(Rout)
    axes[3].plot(t_us, data['V_cm_in']*1e3, 'r', lw=1)
    axes[3].set_ylabel('Voltage (mV)')
    axes[3].set_ylim(550, 850)
    axes[3].set_title('V(Rout)', fontsize=11)
    axes[3].grid(True, alpha=0.3)

    # Panel 5: V(Rout1)
    axes[4].plot(t_us, data['V_out']*1e3, 'b', lw=1)
    axes[4].set_ylabel('Voltage (mV)')
    axes[4].set_ylim(0, max(100, np.max(data['V_out'])*1e3*1.2))
    axes[4].set_title('V(Rout1)', fontsize=11)
    axes[4].grid(True, alpha=0.3)

    # Panel 6: I(Rout1) in nA
    axes[5].plot(t_us, np.abs(data['I_Rout1'])*1e9, 'b', lw=1)
    axes[5].set_ylabel('Current (nA)')
    axes[5].set_xlabel('Time (µs)')
    axes[5].set_ylim(0, max(100, np.max(np.abs(data['I_Rout1']))*1e9*1.2))
    axes[5].set_title('I(Rout1)', fontsize=11)
    axes[5].grid(True, alpha=0.3)

    plt.tight_layout(rect=[0, 0, 1, 0.98])
    plt.savefig(f'{save_path}/neuron_mnist_digit{digit}.png', dpi=300)
    print(f"\nSaved: {save_path}/neuron_mnist_digit{digit}.png")
    plt.show()
